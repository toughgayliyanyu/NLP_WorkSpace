...
class Predictor(object):
    ...
    def load_graph(self):
        ...
        ckpt = tf.train.get_checkpoint_state('./mrpc_output')
        if ckpt and tf.train.checkpoint_exists(ckpt.model_checkpoint_path):
            print('Reloading model parameters..')
            self.model.saver.restore(self.sess, ckpt.model_checkpoint_path)
        else:
            raise ValueError('No such file:[{}]'.format('./mrpc_output'))
...
#####保存模型################
# Predictor().save_model('./resaved_albert_tiny_fp16_3input/resaved_albert_tiny_fp16_3input')
