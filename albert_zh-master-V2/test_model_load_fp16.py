...
class AlbertSentencePair(object):
    def __init__(self, is_training=True):
        ...
        self.vectors_all = tf.placeholder(dtype=tf.float16, shape=[5, 312], name='vectors_all')

        self.built_model()
        self.init_saver()

    def built_model(self):
        bert_config = modeling_fp16.BertConfig.from_json_file(self.__bert_config_path)

        model = modeling_fp16.BertModel(config=bert_config,
                                   is_training=False,
        ...
        index = tf.cast(index, dtype=tf.float16)
...
#####保存模型################
# Predictor().save_model('./resaved_albert_tiny_output_fl16/resaved_albert_tiny_output_fl16')



####创建输入实例进行预测################
vecotr_in=np.load("data_array_5.npy")
print(vecotr_in.shape)
...
# np.savetxt('predictor_0.txt',predictor_0)



# tf.reset_default_graph()
# predictor_1 = Predictor().predict(input_ids, input_mask, segment_ids)
# print('=====',get_cos_similar_matrix(predictor_0,predictor_1))
#
#
# data_array=np.array([[]])
# result_list=[]
# for i in ['基金经理','打篮球','理财产品','工商银行']:
